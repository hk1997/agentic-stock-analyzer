...
@pytest.mark.anyio
async def test_e2e_super_app_flow(unique_users):
    """
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        ...
        # Register User A
        res_a_reg = await client.post("/api/auth/register", json={
        ...
        # Login User A
        res_a_log = await client.post("/api/auth/login", data={
        ...
        # Register User B
        res_b_reg = await client.post("/api/auth/register", json={
        ...
        # Login User B
        res_b_log = await client.post("/api/auth/login", data={
            "username": unique_users["user_b_email"],
            "password": unique_users["password"]
        })
        ...
        link_res = await client.post("/api/auth/link-account", json={"target_email": unique_users["user_b_email"]}, headers=headers_a)
        assert link_res.status_code == 200, f"Link account failed: {link_res.text}"
        
        # Verify User B is in User A's linked accounts
        linked_res = await client.get("/api/auth/linked-accounts", headers=headers_a)
        ...
        exp_get_res = await client.get("/api/finance/expenses", headers=headers_a)
        ...
        ma_res = await client.post("/api/finance/manual-assets", json={
            "asset_type": "cash",
        ...
        nw_res = await client.get("/api/finance/net-worth-history", headers=headers_a)
        ...
        # First, User A creates a portfolio (or uses default if auto-created)
        port_a_list = await client.get("/api/portfolio", headers=headers_a)
        if not port_a_list.json():
            # Create one if not exists
            await client.post("/api/portfolio", json={"name": "A's Portfolio"}, headers=headers_a)
            port_a_list = await client.get("/api/portfolio", headers=headers_a)
        port_a_id = port_a_list.json()[0]["id"]

        # User A adds AAPL
        add_aapl = await client.post(f"/api/portfolio/{port_a_id}/holdings", json={
            "ticker": "AAPL",
            "shares": 10,
        ...
        port_b_list = await client.get("/api/portfolio", headers=headers_b)
        if not port_b_list.json():
            await client.post("/api/portfolio", json={"name": "B's Portfolio"}, headers=headers_b)
            port_b_list = await client.get("/api/portfolio", headers=headers_b)
        port_b_id = port_b_list.json()[0]["id"]

        add_msft = await client.post(f"/api/portfolio/{port_b_id}/holdings", json={
            "ticker": "MSFT",
            "shares": 5,
        ...
        unified_res = await client.get("/api/finance/unified-portfolio", headers=headers_a)
