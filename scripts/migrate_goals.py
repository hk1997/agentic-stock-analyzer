...
async def migrate():
    ...
    async with engine.begin() as conn:
        try:
            await conn.execute(text("ALTER TABLE financial_goals ADD COLUMN IF NOT EXISTS income_sources VARCHAR(500);"))
            print("Successfully added income_sources column to financial_goals table.")
